...
export class ScrapService {
  ...
  async scrapPost(postId: number, sessionUser: IUserWithoutPassword): Promise<ScrapsEntity> {
    ...
    const isAlreadyScraped = await this.scrapsDAO.existsScrap(userId, postId);
    ...
    post.scrapCounts += 1;
    await this.postsDAO.updatePost(postId, post);

    return createdScrap;
  }

  // 스크랩한 게시물들 조회
  async getScrappedPosts(sessionUser: IUserWithoutPassword, paginationQueryDto: PaginationQueryDto): Promise<IPaginatedResponse<any>> {
  ...
  // 특정 게시물 스크랩 취소
  async deleteScrappedPost(postId: number, sessionUser: IUserWithoutPassword): Promise<{ message: string }>{
    const { userId } = sessionUser;
    ...
    if (result.affected === 0) {
      throw new NotFoundException(`스크랩 취소에 실패했습니다.`);
    }

    // 게시물의 스크랩 수 감소
    const post = await this.postsDAO.findPostById(scrapPost.postId);
    if (!post) {
      throw new NotFoundException(`게시물을 찾을 수 없습니다.`);
    }

    post.scrapCounts -= 1;
    await this.postsDAO.updatePost(post.postId, post);
